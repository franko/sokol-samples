...
void frame(int w_int, int h_int, Uint32 frame_duration) {
    /* NOTE: the vs_params_t struct has been code-generated by the shader-code-gen */
    vs_params_t vs_params;
    const float w = w_int;
    const float h = h_int;
    const float t = frame_duration * 0.01;
...
int main(int argc, char **argv) {
    ...
    state.window = SDL_CreateWindow("Window",
        ...
        720, 720, SDL_WINDOW_OPENGL|SDL_WINDOW_ALLOW_HIGHDPI);
    state.ctx = SDL_GL_CreateContext(state.window);

    init();

    const Uint32 frame_duration = 1000 / 30;
    while(!SDL_QuitRequested()){
        ...
        SDL_Delay(frame_duration - (SDL_GetTicks() - t));
    }
